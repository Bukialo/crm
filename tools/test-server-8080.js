// Servidor de testing en puerto 8080 (para evitar conflictos)
const express = require("express");
const cors = require("cors");

const app = express();
const PORT = 8080; // Puerto diferente para evitar conflictos
...
app.get("/", (req, res) => {
  res.json({
    name: "Bukialo CRM API - Test Server (Port 8080)",
    version: "1.0.0-test",
    status: "running",
    port: PORT,
    message: "This is the TEST server without authentication",
...
app.get("/api", (req, res) => {
  res.json({
    ...
    version: "1.0.0-test",
    status: "all systems operational - NO AUTHENTICATION REQUIRED",
    port: PORT,
    availableEndpoints: [
      "GET /api/contacts",
      "POST /api/contacts",
...
// ============================================
// CONTACTS ENDPOINTS - SIN AUTENTICACIÓN
// ============================================
...
app.get("/api/contacts", (req, res) => {
  try {
    ...
    console.log(
      `📋 Getting contacts - Page: ${page}, Size: ${pageSize} - NO AUTH REQUIRED`
    );
    ...
    const response = {
      ...
      message: `Found ${filtered.length} contacts (TEST SERVER - NO AUTH)`,
...
app.post("/api/contacts", (req, res) => {
  try {
    ...
    console.log(`👤 Creating contact (NO AUTH):`, contactData);
    ...
    res.status(201).json({
      success: true,
      data: newContact,
      message: "Contact created successfully (TEST SERVER)",
    });
  } catch (error) {
...
app.get("/api/contacts/:id", (req, res) => {
  try {
    ...
    console.log(`🔍 Getting contact: ${id} (NO AUTH)`);
    ...
    res.json({
      success: true,
      data: contact,
      message: "Contact found (TEST SERVER)",
    });
  } catch (error) {
...
app.put("/api/contacts/:id", (req, res) => {
  try {
    ...
    console.log(`✏️ Updating contact: ${id} (NO AUTH)`);
    ...
    res.json({
      ...
      message: "Contact updated successfully (TEST SERVER)",
    });
  } catch (error) {
...
app.delete("/api/contacts/:id", (req, res) => {
  try {
    ...
    console.log(`🗑️ Deleting contact: ${id} (NO AUTH)`);
    ...
    res.json({
      success: true,
      message: "Contact deleted successfully (TEST SERVER)",
...
app.get("/api/ai/test", (req, res) => {
  res.json({
    success: true,
    message: "AI service is working (TEST SERVER - NO AUTH)",
    port: PORT,
    timestamp: new Date().toISOString(),
  });
});
  ...
  res.json({
    ...
    data: {
      message: {
        id: `msg_${Date.now()}`,
        role: "assistant",
        content: `Recibí tu consulta: "${query}". El sistema está funcionando correctamente (TEST SERVER).`,
        timestamp: new Date().toISOString(),
        metadata: { type: "text", server: "test-8080" },
      },
      suggestions: [
...
app.get("/api/dashboard", (req, res) => {
  res.json({
    ...
    message: "Dashboard data (TEST SERVER - NO AUTH)",
    server: "test-server-8080",
  });
});
...
app.use((req, res) => {
  ...
  res.status(404).json({
    ...
    method: req.method,
    server: "test-server-8080",
    message: "This is the TEST server running on port 8080",
    availableRoutes: [
      "GET /api/contacts",
      "POST /api/contacts",
      "GET /api/contacts/:id",
...
app.listen(PORT, () => {
  ...
  console.log("");
  console.log("🔧 Para cambiar tu frontend a este servidor:");
  console.log(
